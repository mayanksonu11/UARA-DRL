...
def run_episodes(opt, sce, agents, scenario): 
    ...
    Conn_State = torch.zeros(opt.nagents)
    ...
    while nepisode < opt.nepisodes:
        ...
        while nstep < opt.nsteps:            
            ...
            for i in range(opt.nagents):
                Conn_State[i], reward[i], layers[i] = agents[i].Get_Reward(action, action[i], state, scenario)  # Obtain reward and next state
                next_state[i] = Conn_State[i]
            for i in range(opt.nagents):
